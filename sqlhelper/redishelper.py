...
class redishelper:
    ...
    # 插入新的评论地址
    def insert_new_comment_url(self, url):
        try:
            r = redis.Redis(connection_pool=self.redispool)
            if not r.sismember('downloaded_comment', url):
                r.sadd('downloading_comment', url)
            return True
        except:
            traceback.print_exc()
            return False
    ...
    # 取得待爬取的评论地址
    def get_downloading_comment(self):
        try:
            r = redis.Redis(connection_pool=self.redispool)
            if r.scard('downloading_comment') > 0:
                url = str(r.spop('downloading_comment'))[2:-1]
                print('即将访问%s' % url)
                r.sadd('downloaded_comment', url)
                return url
            return None
        except:
            traceback.print_exc()
    ...
    # 取得未爬取的评论数量
    def get_downloading_comment_count(self):
        try:
            r = redis.Redis(connection_pool=self.redispool)
            return r.scard('downloading_comment')
        except:
            traceback.print_exc()
            return 0
    # 插入新的评论照片
    def insert_new_imgs_to_download(self, imgs):
        try:
            r = redis.Redis(connection_pool=self.redispool)
            pipe = r.pipeline(transaction=True)
            for img in imgs:
                r.sadd('comment_img', img)
            pipe.execute()
            print('添加了%s张图片等待下载' % str(len(imgs)))
            return True
        except:
            return False
    #取得待下载的评论照片
    def get_comment_img_to_download(self):
        try:
            r = redis.Redis(connection_pool=self.redispool)
            if r.scard('comment_img') <= 0:
                return None
            else:
                return str(r.spop('comment_img'))[2:-1]
        except:
            traceback.print_exc()
            return None
    # 取得待下载的评论照片数量
    def get_comment_img_count(self):
        try:
            r = redis.Redis(connection_pool=self.redispool)
            return r.scard('comment_img')
        except:
            traceback.print_exc()
            return 0
